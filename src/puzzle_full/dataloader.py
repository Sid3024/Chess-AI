...
class ChessIterableDataset(IterableDataset):
    def __init__(self, db_path, n_limit, token_encoding_scheme, puzzle_complexity, index_to_move, move_to_index, masking=False, min_rating=0):
        ...
        self.moves_to_solve = puzzle_complexity / 2
    ...
    def data_generator(self):
        ...
        for row in cursor.fetchall():
            ...
            puzzle_moves = json.loads(row[1])
            for i in range(0, self.puzzle_complexity, 2):
                opponent_move = puzzle_moves[i]
                board.push(chess.Move.from_uci(self.index_to_move[opponent_move]))
                fen = board.fen()
                board_state, special_tokens, turn_encoding = self.fen_to_vector(fen)
                board_state_list.append(board_state)
                special_tokens_list.append(special_tokens)                
                target_move_index = puzzle_moves[i+1]
                if turn_encoding == 1 and self.token_encoding_scheme % 2 == 0:
                    target_move_list.append(self.flip_uci(target_move_index))
                else:
                    target_move_list.append(target_move_index)
                # if turn_encoding == 1 and self.token_encoding_scheme % 2 == 0:
                #     flipped_target_move = torch.tensor(self.flip_uci(target_move_index), dtype=torch.int64)
                # else:
                #     flipped_target_move = torch.tensor(target_move_index, dtype=torch.int64)
                board.push(chess.Move.from_uci(self.index_to_move[target_move_index]))

                
            board_state_tensor, special_token_tensor, target_move_tensor = torch.stack(board_state_list, dim=0), torch.stack(special_tokens_list, dim=0), torch.tensor(target_move_list, dtype=torch.int64)
    ...
    def fen_to_vector(self, fen):
        ...
        if self.token_encoding_scheme % 2 == 1:
        ...
        return torch.tensor(position, dtype=torch.int64), torch.tensor(special_tokens, dtype=torch.int64), turn_encoding #, half_moves, full_moves
