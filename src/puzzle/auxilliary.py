...
def retrieve_iteration_number(model_iteration, min_puzzle_rating, write=True):
    iteration = 0
    with open("/workspace/src/puzzle_tests/lichess/completed_indices.txt", "a+") as file:
        file.seek(0)
    ...
    if write:
        with open(f"/workspace/src/puzzle_tests/lichess/model_{iteration}.txt", 'w') as file:
            file.write(f"model_iter: {model_iteration}\n")
            file.write(f"min_puzzle_rating: {min_puzzle_rating}\n")
    return iteration

    
def write_to_file(log_path, total_params, HyperParamConfig, VariableRunConfig, DataConfig):
